 ...
 */
export function createOrchestratorHandlers(scheduler, pool) {
    const app = new Hono();
    // CORS middleware
    ...
    app.post("/orchestrator/queue", async (c) => {
        try {
            ...
            await Effect.runPromise(scheduler.runNow(taskId, "browser-automation", params, {
...
function createErrorResponse(error) {
    ...
    const errorTag = error?._tag || "UnknownError";
    return {
        error: errorMessage,
        tag: errorTag,
    };
