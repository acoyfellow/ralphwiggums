 ...
 */
function executeTaskOnBrowser(task, browserUrl) {
 ...
 */
export function dispatchTasks(scheduler, pool) {
    return Effect.gen(function* () {
        ...
        yield* Effect.all(tasksToProcess.map(task => dispatchSingleTask(task, pool)), { concurrency: availableCapacity });
    });
}
/**
 * Dispatch a single task to an available browser
 */
function dispatchSingleTask(task, pool) {
    return Effect.gen(function* () {
