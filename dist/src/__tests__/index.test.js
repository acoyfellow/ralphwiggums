...
describe("RalphWiggums Core", () => {
    describe("doThis()", () => {
        it("should return successful result on first iteration", async () => {
            ...
            const result = await Effect.runPromise(Ralph.doThis("Click submit button"));
        ...
        it("should return success when response includes 'success' string", async () => {
            ...
            const result = await Effect.runPromise(Ralph.doThis("Fill form"));
            expect(result.success).toBe(true);
            expect(result.iterations).toBe(1);
        });
        it("should complete after two iterations when first fails", async () => {
            ...
            const result = await Effect.runPromise(Ralph.doThis("Fill form", { maxIterations: 5 }));
            expect(result.success).toBe(true);
        ...
        it("should respect maxIterations option", async () => {
            ...
            mockContainerFetch.mockImplementation((path) => {
                ...
                if (path === "/start")
                    return Promise.resolve({ success: true });
                if (path === "/instruction")
                    return Promise.resolve({ data: "Still working..." });
                return Promise.resolve({ success: true });
            });
...
describe("RalphWiggums Integration Scenarios", () => {
    ...
    it("should complete form filling workflow", async () => {
        ...
        const result = await Ralph.run("Fill out the contact form with name=test, email=test@example.com");
        expect(result.success).toBe(true);
        expect(result.iterations).toBe(1);
        expect(result.checkpointId).toBeDefined();
    ...
    it("should track iterations correctly with multi-step task", async () => {
        ...
        mockContainerFetch.mockImplementation((path) => {
            ...
            if (path === "/instruction") {
                if (callCount <= 6)
                    return Promise.resolve({ data: "Step in progress..." });
                return Promise.resolve({ data: "TASK_COMPLETE" });
            }
            return Promise.resolve({ success: true });
        });
        const result = await Effect.runPromise(Ralph.doThis("Multi-step task", { maxIterations: 5 }));
        expect(result.success).toBe(true);
